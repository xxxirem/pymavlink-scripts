...
def set_ahrs_origin(master_instance, lat=55.7558, lon=37.6173, alt=0.0):
    # 1. Отправка EKF Origin (нужно для работы без GPS)
    master_instance.mav.set_gps_global_origin_send(
        master_instance.target_system,
        int(lat * 1e7),
        int(lon * 1e7),
        int(alt * 1000),
    )
    print("[AHRS Origin] Message SET_GPS_GLOBAL_ORIGIN send.")

    # 2. Фиксация точки Home
    master_instance.mav.set_home_position_send(
        master_instance.target_system,
        int(lat * 1e7),
        int(lon * 1e7),
        int(alt * 1000),
    ...
    print("[Home Position] Message SET_HOME_POSITION send.")
...
def main():
    ...
    time.sleep(1)  # Даем EKF свести координаты
